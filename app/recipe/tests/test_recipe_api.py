...
class PrivateRecipeAPITest(TestCase):
    ...
    def test_recipe_list_limited_to_user(self):
        ...
        other_user= create_user(email='user2@example.com', password='random123')
        create_recipe(user=self.user)
    ...
    def test_create_recipe_with_new_ingredient(self):
        ...
        for ing in payload['ingredients']:
            self.assertTrue(recipe.ingredients.filter(user=self.user, name=ing['name']).exists())

    def test_create_recipe_with_existing_ingredients(self):
        '''Test to create recipe with existing ingredients'''
        old_ing=Ingredient.objects.create(user=self.user, name='Old ing')
        payload={
            'title':'Title',
            'link':'https://xyz.com',
            'time_minutes':25,
...
class ImageUploadTests(TestCase):
    ...
    def setUp(self):
        self.user=create_user(email='test@example.com', password='random123')
        self.client=APIClient()
        self.client.force_authenticate(self.user)
